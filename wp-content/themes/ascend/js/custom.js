jQuery(document).ready(function(){
	...
	jQuery(window).on('load',function () {		
    	jQuery('.main_items li.opened').find('.sub_items').css({'display':'block'});
	});

	var cDiv = jQuery( '.mainSidebar.for_stky .asideContainer' );
	var footer_= jQuery('footer');	  
	if (cDiv.length) {
	  	var contentOffset = cDiv.offset().top;	  
	  	var winWid =jQuery(window).width();
		var topPos = contentOffset - jQuery('#main_header').height();
			jQuery(window).scroll(function () {	
				var current_height = jQuery('.productContent.push_right').height();

				console.log('div height: '+jQuery('.productContent.push_right').height());

				 var winSc = jQuery(this).scrollTop();

				 if(winWid > 767){

				 	var current_offset = (cDiv.offset().top) + (cDiv.height());

				 	console.log('current Offset: '+current_offset);
				 	
					 if(winSc > topPos){
					 	if((winSc+200) > current_height){
					 		jQuery( '.mainSidebar.for_stky .asideContainer' ).css({'position':'absolute','top': (current_height-cDiv.height())+'px','z-index':'9999', 'width':230+'px'});
							
					 	}else{
					 		//alert();
					 		jQuery( '.mainSidebar.for_stky .asideContainer' ).css({'position':'fixed','top': 114+'px','z-index':'9999', 'width':230+'px'});
						...
						jQuery( '.mainSidebar.for_stky .asideContainer' ).css({'position':'inherit','top': '0px'});
					}
        ...
        jQuery('.main_items li a').each(function(){
             ...
             // alert(ff);
              ...
              if(currentElem.length){
              	var currE = currentElem.position().top;
              }
              if( currE <= st + headHeight) {
...
//alert('targetElement');
	...
	jQuery(document).on('click',function(){
	 ...
	 jQuery(document).on('click', '.btnHolder .btn.btn_readMore' ,function(arguments) {
...
alert(jQuery('.productRow ').length);
jQuery('.productRow').each(function(argument) {
	var ratings = parseInt(jQuery(this).find('.ratingHolder .ratingPoint').text());
	//console.log('ratings: '+ratings);
	jQuery(this).find('.ratingArea ul li').addClass('doneRating');
});
...
jQuery(function(){
	...
	var h = jQuery(window).height();
	var lastScrollTop = 0;
	var sContent = jQuery('#mid_menu, #order_mid_menu');
	if(sContent.length){
		var sContent_offset = sContent.offset().top;
	}
	
	var stickyMenu= sContent_offset;
	//alert(stickyMenu)
	jQuery(window).scroll(function(event){
	   var st = jQuery(this).scrollTop();
	   if ((st) > lastScrollTop){
