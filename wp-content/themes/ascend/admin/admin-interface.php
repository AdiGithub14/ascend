...
function optionsframework_add_admin() {
    ...
    $of_page = 
    add_theme_page( 'Theme Option Panel', 'Theme Options', 'edit_theme_options', 'optionsframework','optionsframework_options_page' );
...
<script type="text/javascript" language="javascript">
		...
		<script type="text/javascript">
			...
			jQuery('#ofform').submit(function(){
				...
				jQuery('.section-webq_editor .option .explain').remove();
				jQuery('.section-webq_editor .option .controls').removeClass('controls');
...
function optionsframework_machine($options) {
	...
	foreach ($options as $value) {
		...
		case 'webq_editor':
			...
			$output .= get_wp_editor_wq($val , $value['id'], array('textarea_rows' => 8));
		break;
...
function get_wp_editor_wq($val , $value_id, $settings){
	
	ob_start();

	// Echo the editor to the buffer
	wp_editor($val , $value_id, $settings);
